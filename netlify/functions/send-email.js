exports.handler = async function(event, context) {
  ...
  try {
    ...
    const { name, phone, email, goal, date, time, plan, addons, total } = data;
    ...
    const subject = isPlanOrder 
      ? `🛍️ Nuevo Plan Solicitado: ${plan} - ${name}`
      : `⚡ Nuevo Lead: ${name}`;
      ...
      emailHtml = `
        <div style="font-family: sans-serif; padding: 24px; color: #111; max-width: 600px; margin: 0 auto; border: 1px solid #145bff; border-radius: 16px; box-shadow: 0 4px 16px rgba(20,91,255,0.08); background-color: #ffffff;">
          ...
          <div style="padding: 24px 0;">
            <h3 style="color: #145bff; border-bottom: 2px solid #f3f4f6; padding-bottom: 6px; margin-top: 0;">Detalles del Cliente</h3>
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
              <tr style="background-color: #f9fafb;">
                ...
                <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; color: #111827; font-size: 14px; font-weight: 500;">${name}</td>
              </tr>
              <tr>
            ...
            <div style="background-color: #0d1220; border: 1px solid #1e293b; border-radius: 12px; padding: 18px; color: white;">
